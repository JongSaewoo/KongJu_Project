...
<html>
 <% 
	...
	if(boardID == null || boardID.equals("")) {
  	...
  	BoardDTO board = boardDAO.getBoard(boardID);  //해당 게시글 DTO만 가져오도록
  	if(board.getBoardAvailable() == 0){
  		session.setAttribute("messageType", "오류 메세지");
   		session.setAttribute("messageContent", "삭제된 게시글 입니다.");
  	...
  	if(session.getAttribute("userID").equals("admin") || session.getAttribute("userID").equals(board.getUserID()) ){  //관리자만 읽을 수 있다.
    	
  	}else {
  		session.setAttribute("messageType", "오류 메세지");
   		session.setAttribute("messageContent", "관리자와 본인만 읽을 수 있는 글입니다.");
   		response.sendRedirect("boardView.jsp");
   		return;
  	}
