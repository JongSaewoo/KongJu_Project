		...
		<table class="table table-bordered table-hover" style="text-align:center; border: 1px solid #dddddd">
			<thead>
				...
				<tr>
					...
					<th style="background-color: #fafafa; color: #000000; width: 100px;"><h5>첨부파일</h5></th>
				</tr>
			</thead>
			<tbody>
			<%
			...
			%>		
					...
					ㄴRe   <span class="glyphicon glyphicon-arrow-right" aria-hidden="true"> </span>
			<%
				}
			%>	
			<%
			...
			%> 
				<span style="color:#92B3B7" > -- 삭제된 게시물 입니다. -- ></span>
			<%
				}else {
			%>
			     <%= board.getBoardTitle() %>    
			...
			%>
					...
					<%
					   if(board.getBoardFile().trim().equals("") ){
					%>
...
<!-- 						부트스트랩 제공 페이지네이션 -->
						...
						<%
							...
							}else{ //1이 아닌경우 링크없이 이동할수 없음으로 설정, 일반적으로 색깔을 선택할 수 없음을 알릴때는 style="color: gray;로 설정
						%>
						...
						<%
							for(int i= Integer.parseInt(pageNumber)+1; i <= targetPage +Integer.parseInt(pageNumber); i++){
								...
								if(i< startPage +10){ //시작페이지에 10을 더한것보다 작은경우
						%>
							<li><a href="boardView.jsp?pageNumber=<%= i%>" ><%=i%></a></li>
						<%
								}
